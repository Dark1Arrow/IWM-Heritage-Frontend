...
import { 
  ...
  Info, Camera, Languages, Phone, Globe, Mail 
} from "lucide-react";
...
const HeritageDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      setLoading(true);
      try {
        const result = await dispatch(getHeritageDetails(heritageId));
        setHeritage(result?.data || result);
      } catch (err) {
    ...
    fetchDetails();
  }, [heritageId, dispatch]); // Corrected dependency
